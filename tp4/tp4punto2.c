...
void convertir(int in_fd, int out_fd, BMPHeader h, BMPInfoHeader infoh,int comienzoFila,int finFila){
...
int main() 
{
    ...
    //Separar convertir en 3 proceso
  	...
  	if(a==0){
  	    ...
  	    int b=fork();
  	    if(b==0){
      	    convertir(in_fd, out_fd, h, infoh,354,708);
      	    exit(0);
  	    }else{
  	        int c=fork();
  	        if(c==0){
  	            convertir(in_fd, out_fd, h, infoh,708,1063);
          	    exit(0);
  	        }else{
  	            wait(a);
  	            wait(b);
